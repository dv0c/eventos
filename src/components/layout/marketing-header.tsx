...
export function MarketingHeader({ className }: { className?: string }) {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 w-full border-b border-white/10 bg-black/35 backdrop-blur-xl",
        className,
      )}
    >
      <div className="mx-auto flex h-[4.25rem] max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
          ...
          <Link
            href="/login"
            className="hidden px-2.5 text-[15px] font-medium text-white/70 transition-colors hover:text-white sm:inline-flex"
          >
            {tAuth("signIn")}
          </Link>
          <Button
